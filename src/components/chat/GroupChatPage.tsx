    ...
    <div ref={swipeRef} data-group-page className="fixed inset-0 z-[70] bg-app flex min-h-0 flex-col overflow-hidden"
      ...
      {view === 'list' && (
        <div data-group-header className="min-h-14 flex items-center gap-2 px-3 py-2 border-b border-line shrink-0">
          ...
          <span className="text-sm font-medium text-ink flex-1">群聊</span>
          <button onClick={() => setCreating(true)} className="shrink-0 px-3 py-1.5 rounded-lg text-xs text-gene-purple hover:bg-gene-purple/10 transition-colors">发起群聊</button>
        </div>
      )}
      ...
      {view === 'list' ? (
        ...
        <GroupChatWindow onBack={() => { setView('list'); void loadGroups(); }} />
      )}
