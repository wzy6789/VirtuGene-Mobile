...
import { transcribeWithSiliconFlow, getCloudAsrKey } from '../../lib/cloud-asr';
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({ onSend, onSendImage, onSendVoice, disabled }, ref) {
  ...
  const finishAndSend = async () => {
    ...
    // 系统识别无结果 → 尝试云端识别（内置默认 Key 开箱即用；OPPO 等无系统引擎的手机自动走云端）
    let text = sysText;
    if (result && !text) {
      const key = await getCloudAsrKey();
      if (key) {
        try {
    ...
    if (!result || !text) {
      ...
      if (!result) msg = '录音失败';
        ...
        msg = avail ? '没听清，请再说一次' : '语音识别失败（云端不可用），请重试或用键盘输入';
      }
      showToast(msg, 2600);
      return;
    }
