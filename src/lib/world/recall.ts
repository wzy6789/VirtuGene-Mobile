...
export async function selectRecallableSharedMemories(params: {
  ...
  const candidates = await sharedMemoryRepo.listRelevant(worldId, [characterId], Math.max(limit * 4, 12));
  if (candidates.length === 0) return [];
  const candidateIds = new Set(candidates.map((m) => m.id));

  // 认知闸门：这个角色**确实知道并且可以主动提起**的世界事件
  const known = await knowledgeRepo.listKnownBy(characterId, worldId, { minLevel: 'full', limit: 200 });
  ...
  const events = await worldEventRepo.listVisibleToCharacter(worldId, characterId, 200);
